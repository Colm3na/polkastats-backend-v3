...
const makeQuery = (state, block, timestamp) => {
  ...
  const availableBalance = getAmount(balances.availableBalance.toString());
  const freeBalance = getAmount(balances.freeBalance.toString());
 ...
 * @param {*} sequelize  Postgres connection pool
 */
const exec = async (api, sequelize) => {
  ...
  await Promise.all(
    makeState([accountIds, accountsIdentity, accountsBalances, accountsStaking])
      ...
      .map(state => execQuery(state, sequelize))
