  ...
  try {
    ...
    const unsubscribe = await api.rpc.chain.subscribeNewHeads(async header => {
      ...
      if (res.rows.length > 0) {
        ...
        console.log(
          `[PolkaStats backend v3] - Block listener - \x1b[33mDetected chain reorganization at block #${blockNumber}, updating author, author name, hash and state root\x1b[0m`
        );
        ...
        blockEvents.forEach(async (record, index) => {
          ...
          try {
            await pool.query(sqlInsert);
            console.log(
              `[PolkaStats backend v3] - Block listener - \x1b[33mAdding event #${blockNumber}-${index} ${event.section} => ${event.method}\x1b[0m`
            );
          } catch (error) {
            console.log(
              `[PolkaStats backend v3] - Block listener - \x1b[31mError adding event #${blockNumber}-${index}: ${error.error}\x1b[0m`
            );
          }
        });
      }
    ...
    console.log("Error running Listener: ", error);
  }
}
