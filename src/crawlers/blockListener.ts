...
import { shortHash, storeExtrinsics, storeLogs, getDisplayName, updateTotals } from '../lib/utils'
...
async function start(api: ApiPromise, pool: Pool, _config: any): Promise<any> {
  logger.info(loggerOptions, `Starting block listener...`);
    ...
    await api.rpc.chain.subscribeNewHeads(async (header:any) => {
      ...
      if (res.rows.length > 0) {
        ...
        const timestampMs: any = await api.query.timestamp.now.at(blockHash);
