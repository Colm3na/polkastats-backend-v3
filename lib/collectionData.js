...
async function get(collectionId, api) {
  const source = await api.query.nft.collectionById(collectionId);  
  const collection = toObject(source);    
...
function getData( collection ) {
  ...
  result.name = bufferToString(collection.Name);
  result.description = bufferToString(collection.Description);
