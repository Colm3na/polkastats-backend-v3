...
async function del(tokenId, collectionId, sequelize, transaction = null) {
...
function setReplacements(aToken) {
  ...
  result.owner_normalized = normalizeSubstrateAddress(aToken.owner);
  return result;
}
...
async function save({
  ...
  await sequelize.query(
    ...
    `,
    {
      ...
      logging: false,
      replacements: setReplacements(token),
      transaction,
    },
  );
}
