...
const logger = pino();
const { storeEraRewardsSlashes } = require('../utils.js');
...
module.exports = {
  start: async function (api, pool, _config, substrateNetwork) {
    ...
    if (res.rows.length > 0) {
      ...
      await storeEraRewardsSlashes(api, pool, currentEraIndex - 1, denom);
    }
    ...
    await api.rpc.chain.subscribeNewHeads( async () => {
      ...
      if (currentEraIndex > currenKnownEraIndex) {
        currenKnownEraIndex = currentEraIndex;
        await storeEraRewardsSlashes(api, pool, currentEraIndex - 1);
      }
    });
  }
}