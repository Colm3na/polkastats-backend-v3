  ...
  start: async function(api, pool, config) {
    ...
    for (let i = 0; i < res.rows.length; i++) {
      ...
      if (!(res.rows[i].gap_start == 0 && res.rows[i].gap_end == 0)) {
        logger.info(`Detected gap! Harvesting blocks from #${res.rows[i].gap_end} to #${res.rows[i].gap_start}`);
    ...
    // Log execution time
    //
    logger.info(`Added ${addedBlocks} blocks in ${((endTime - startTime) / 1000).toFixed(0)}s`);
    logger.info(`Next execution in 60m...`);
    ...
    while (endBlock >= startBlock) {
      ...
      blockEvents.forEach( async (record, index) => {
        ...
        try {
          ...
          logger.info(`Added event #${endBlock}-${index} ${event.section} => ${event.method}`);
        } catch (error) {
          logger.info(`Error adding event #${endBlock}-${index}: ${error}, sql: ${sqlInsert}`);
        }
      });
      ...
      if (currentEraStartSessionIndex) {
        ...
        if (res.rows.length > 0) {
          ...
          try {
            ...
            logger.info(`Error updating validator_produced_blocks: ${JSON.stringify(error)}`);
          }
        } else {
        ...
        try {
          await pool.query(sqlInsert);
          const endTime = new Date().getTime();
          logger.info(`Added block #${endBlock} (${shortHash(blockHash.toString())}) in ${((endTime - startTime) / 1000).toFixed(3)}s`);
        } catch (error) {
          logger.info(`Error adding block #${endBlock}: ${error.error}`);
        }
        endBlock--;
        addedBlocks++;
      } else {
        logger.info(`End of history depth reached, stopping!`);
      }
    }
  }
}