...
module.exports = {
  ...
  harvestBlocks: async function(api, pool, startBlock, endBlock) {
    ...
    while (endBlock >= startBlock) {
      ...
      const currentEraStartSessionIndex = await api.query.staking.erasStartSessionIndex(currentEra.toString());
      ...
      if (currentEraStartSessionIndex) {
        ...
        const eraProgress = currentIndex.sub(currentEraStartSessionIndex.toString()).mul(epochDuration).add(sessionProgress);
