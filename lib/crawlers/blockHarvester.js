    ...
    while (endBlock >= startBlock) {
      ...
      try {
        ...
        if (block) {
          ...
          const blockAuthorIdentity = await api.derive.accounts.info(author);
          ...
          // Store block logs
          await storeLogs(pool, endBlock, block.header.digest.logs, loggerOptions);
        ...
        logger.error(loggerOptions, `Error getting block ${endBlock} (${blockHash}): ${error}`);
        try {
          const timestamp = new Date().getTime();
          const sql = `INSERT INTO failed_block (block_number, error, timestamp)
            VALUES ('${endBlock}', '${error.toString().replace(/'/g, "''")}', '${timestamp}')
            ON CONFLICT ON CONSTRAINT failed_block_pkey DO NOTHING
          `;
          logger.error(loggerOptions, `SQL: ${sql}`);
