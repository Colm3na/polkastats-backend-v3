  ...
  harvestBlocks: async function(api, pool, startBlock, endBlock) {
    ...
    while (endBlock >= startBlock) {
      ...
      const [
        ...
        new BigNumber(api.query.session.currentIndex.at(blockHash)),
        new BigNumber(api.query.babe.currentSlot.at(blockHash)),
        new BigNumber(api.query.babe.epochIndex.at(blockHash)),
        new BigNumber(api.query.babe.genesisSlot.at(blockHash)),
        new BigNumber(api.query.staking.currentEra.at(blockHash))
      ]);
      ...
      if (currentEraStartSessionIndex) {
        ...
        if (res.rows.length > 0) {
          ...
          try {
            ...
            logger.error(loggerOptions, `Error updating validator_produced_blocks: ${JSON.stringify(error)}`);
          }
        } else {
          sql = `INSERT INTO validator_produced_blocks (session_index, account_id, produced_blocks, timestamp) VALUES ('${currentIndex}', '${blockAuthor}', '1', '${Date.now()}');`;
          try {
            await pool.query(sql);
          } catch (error) {
            logger.error(loggerOptions, `Error updating validator_produced_blocks: ${JSON.stringify(error)}`);
