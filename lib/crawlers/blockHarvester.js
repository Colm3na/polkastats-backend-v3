...
module.exports = {
  start: async function(api, pool, config) {
    logger.info(`Starting block harvester...`, loggerOptions);
    ...
    for (let i = 0; i < res.rows.length; i++) {
      ...
      if (!(res.rows[i].gap_start == 0 && res.rows[i].gap_end == 0)) {
        logger.info(`Detected gap! Harvesting blocks from #${res.rows[i].gap_end} to #${res.rows[i].gap_start}`, loggerOptions);
    ...
    // Log execution time
    //
    logger.info(`Added ${addedBlocks} blocks in ${((endTime - startTime) / 1000).toFixed(0)}s`, loggerOptions);
    logger.info(`Next execution in 60m...`, loggerOptions);
  ...
  harvestBlocks: async function(api, pool, startBlock, endBlock) {
    ...
    while (endBlock >= startBlock) {
      ...
      await storeExtrinsics(pool, endBlock, block.extrinsics, blockEvents);
      ...
      blockEvents.forEach( async (record, index) => {
        ...
        try {
          ...
          logger.info(`Added event #${endBlock}-${index} ${event.section} => ${event.method}`, loggerOptions);
        } catch (error) {
          logger.error(`Error adding event #${endBlock}-${index}: ${error}, sql: ${sqlInsert}`, loggerOptions);
        }
      });
      ...
      if (currentEraStartSessionIndex) {
        ...
        try {
          await pool.query(sqlInsert);
          const endTime = new Date().getTime();
          logger.info(`Added block #${endBlock} (${shortHash(blockHash.toString())}) in ${((endTime - startTime) / 1000).toFixed(3)}s`, loggerOptions);
        } catch (error) {
          logger.error(`Error adding block #${endBlock}: ${error.error}`, loggerOptions);
        }
        endBlock--;
        addedBlocks++;
      } else {
        logger.info(`End of history depth reached, stopping!`, loggerOptions);
      }
    }
  }
}