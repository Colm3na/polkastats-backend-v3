  ...
  harvestBlocks: async function(api, pool, startBlock, endBlock) {
    ...
    while (endBlock >= startBlock) {
      ...
      if (currentEraStartSessionIndex) {
        ...
        let sql = `SELECT produced_blocks FROM validator_produced_blocks WHERE session_index = '${currentIndex}' AND account_id = '${author}'`;
        let res = await pool.query(sql);
        if (res.rows.length > 0) {
          const producedBlocks = parseInt(res.rows[0].produced_blocks) + 1;
          sql = `UPDATE validator_produced_blocks SET produced_blocks = '${producedBlocks}' WHERE session_index = '${currentIndex}' AND account_id = '${author}'`;
          try {
          ...
          sql = `INSERT INTO validator_produced_blocks (session_index, account_id, produced_blocks, timestamp) VALUES ('${currentIndex}', '${author}', '1', '${Date.now()}');`;
          try {
            await pool.query(sql);
          } catch (error) {
            logger.error(loggerOptions, `Error updating validator_produced_blocks: ${JSON.stringify(error)}`);
