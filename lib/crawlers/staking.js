...
module.exports = {
  ...
  storeSessionStakingInfo: async function (api, pool, blockNumber, sessionInfo) {
    ...
    validatorStaking = await Promise.all(
      validatorAddresses.map(authorityId => api.derive.staking.account(authorityId)).then((validator) => {
        return {
          ...validator,
    ...
    intentionStaking = info.map(intention => {
      return {
        ...intention,
        sessionIdHex: intention.sessionIds.length !== 0 ? intention.sessionIds.toHex() : ``,
        nextSessionIdHex: intention.nextSessionIds.length !== 0 ? intention.nextSessionIds.toHex() : ``,
