...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    //
    const [allStashAddresses, validatorAddresses, imOnline, erasRewardPoints] = await Promise.all([
    ...
    validatorStaking.forEach(async intention => {
      const sql = `INSERT INTO intention_bonded (block_number, session_index, account_id, amount, timestamp) VALUES ('${blockNumber}', '${currentIndex}', '${intention.accountId.toString()}', '${BigNumber(intention.exposure.total).toString(10)}', extract(epoch from now()));`;
      await pool.query(sql);
    })
