...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    //
    const [validatorAddresses, imOnline, stakingOverview, erasRewardPoints] = await Promise.all([
    ...
    for(let i = 0; i < intentionStaking.length; i++) {
      intentionStaking[i].stakers = nominations
        .filter(nomination => nomination.targets.some(target => target === intentionStaking[i].accountId))
