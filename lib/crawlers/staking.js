...
module.exports = {
  async staking(api, pool, _config) {
    console.log(
      `[PolkaStats backend v3] - \x1b[32mStarting staking crawler...\x1b[0m`,
    );
    ...
    await api.rpc.chain.subscribeNewHeads(async header => {
      ...
      if (sessionInfo.currentIndex > currentDBIndex) {
        if (!crawlerIsRunning) {
          await module.exports.storeStakingInfo(
            api,
            pool,
            blockNumber,
            sessionInfo,
  ...
  async storeStakingInfo(api, pool, blockNumber, sessionInfo) {
    ...
    validatorStaking.forEach(validator => {
      validator.sessionIdHex =
        validator.sessionIds.length !== 0 ? validator.sessionIds.toHex() : ``;
      validator.nextSessionIdHex =
        validator.nextSessionIds.length !== 0
          ? validator.nextSessionIds.toHex()
          : ``;
    });
    ...
    for (let i = 0; i < validatorStaking.length; i++) {
      ...
      if (
        currentEraPointsEarned.individual[
          currentElected.indexOf(validator.accountId)
        ]
      ) {
