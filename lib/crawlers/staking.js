...
module.exports = {
  ...
  storeSessionStakingInfo: async function (api, pool, blockNumber, sessionInfo) {
    ...
    const intentionRows = intentionStaking.map(intention => {
      return [
        blockNumber,
        currentIndex,
        intention.accountId,
    ...
    try {
      const sql = format(`
        INSERT INTO intention (
          ...
          next_elected,
          timestamp
        ) VALUES %L`, intentionRows);
      await pool.query(sql);
    } catch (error) {
      logger.error(loggerOptions, `Error inserting data in intention table: ${JSON.stringify(error)}`);
    }
    ...
    const nominatorRows = nominatorStaking
      .sort((a, b) => {
    ...
    try {
      const sql = format(`
        INSERT INTO nominator (
          ...
          targets,
          timestamp
        ) VALUES %L`, nominatorRows);
      await pool.query(sql);
    } catch (error) {
      logger.error(loggerOptions, `Error inserting data in nominator table: ${JSON.stringify(error)}`);
    }
