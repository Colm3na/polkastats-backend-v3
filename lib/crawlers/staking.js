...
module.exports = {
  start: async function (api, pool, _config) {
    logger.info(`Starting staking crawler...`, loggerOptions);
    ...
    if (res.rows.length > 0) {
      ...
      logger.info(`Last session index stored in DB is #${currentDBSessionIndex}`, loggerOptions);
    } else {
      logger.info(`First execution, no session index found in DB!`, loggerOptions);
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    logger.info(`Storing validators staking info for session #${currentIndex} (block #${blockNumber})`, loggerOptions);
    ...
    if (validatorStaking) {
      logger.info(`Inserting staking data in DB`);
      ...
      try {
        ...
        logger.error(`Error inserting staking data in DB: ${JSON.stringify(error)}`, loggerOptions);
      }
    }
    ...
    //
    logger.info(`Populating validator_bonded, validator_selfbonded, validator_num_nominators and validator_active tables`);
    validatorStaking.forEach(async validator => {
      ...
      let sql = `INSERT INTO validator_bonded (block_number, session_index, account_id, amount, timestamp) VALUES ('${blockNumber}', '${currentIndex}', '${validator.accountId.toString()}', '${totalBonded}', extract(epoch from now()));`;
      await pool.query(sql);
      
      // populate validator_selfbonded table
      sql = `INSERT INTO validator_selfbonded (block_number, session_index, account_id, amount, timestamp) VALUES ('${blockNumber}', '${currentIndex}', '${validator.accountId.toString()}', '${selfBonded}', extract(epoch from now()));`;
      await pool.query(sql);

      // populate validator_num_nominators table
      sql = `INSERT INTO validator_num_nominators (block_number, session_index, account_id, nominators, timestamp) VALUES ('${blockNumber}', '${currentIndex}', '${validator.accountId.toString()}', '${numNominators}', extract(epoch from now()));`;
      await pool.query(sql);
    ...
    //
    logger.info(`Storing intentions staking info for session #${currentIndex} (block #${blockNumber}`, loggerOptions);
    ...
    //
    logger.info(`Populating intention_staking table`, loggerOptions);

    if (intentionStaking) {
      logger.info(`Inserting intention staking data in DB`, loggerOptions);
      ...
      try {
        ...
        logger.error(`Error inserting intention staking data in DB: ${JSON.stringify(error)}`, loggerOptions);
      }
    }
    ...
    //
    logger.info(`Populating intention_bonded table`, loggerOptions);
    intentionStaking.forEach(async intention => {
      const sql = `INSERT INTO intention_bonded (block_number, session_index, account_id, amount, timestamp) VALUES ('${blockNumber}', '${currentIndex}', '${intention.accountId.toString()}', '${new BigNumber(intention.stakingLedger.total).toString(10)}', extract(epoch from now()));`;
      await pool.query(sql);
    })
