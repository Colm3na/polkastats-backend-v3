...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    // Add identity
    //
    validatorStaking.forEach(async function (validator) {
    ...
    for(let i = 0; i < validatorStaking.length; i++) {
      ...
      const sql = `
        INSERT INTO validator (
          ...
          '${validator.staking_ledger}',
          '${JSON.stringify(validator.validator_prefs)}',
          '${validator.validator_prefs.commission}',
    ...
    // Add identity
    //
    intentionStaking.forEach(async function (intention) {
    ...
    for(let i = 0; i < intentionStaking.length; i++) {
      ...
      const sql = `
        INSERT INTO intention (
          ...
          '${intention.staking_ledger}',
          '${JSON.stringify(intention.validator_prefs)}',
          '${intention.validator_prefs.commission}',
