...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    //
    // Get active validators, imOnline data and next elected
    //
    ...
    logger.info(loggerOptions, `Got active validators, imOnline data and next elected in ${((endTime1 - startTime) / 1000).toFixed(3)}s`);
    ...
    logger.info(loggerOptions, `Got validator staking info in ${((endTime2 - endTime1) / 1000).toFixed(3)}s`);
    ...
    logger.info(loggerOptions, `Got validator imOnline info in ${((endTime3 - endTime2) / 1000).toFixed(3)}s`);
    ...
    logger.info(loggerOptions, `Got validator identity info in ${((endTime4 - endTime3) / 1000).toFixed(3)}s`);
      ...
      if (validator.exposure.others.length > 0) {
        for (let j = 0; j < validator.exposure.others.length; j++) {
          ...
          if (
            ...
            const [{ identity }, balances, bonded] = await Promise.all([
              api.derive.accounts.info(exposure.who),
              api.derive.balances.all(exposure.who),
              api.query.staking.bonded(exposure.who)
            ]);
    ...
    logger.info(loggerOptions, `Got and store validator nominator info in ${((endTime5 - endTime4) / 1000).toFixed(3)}s`);
    ...
    logger.info(loggerOptions, `Populated validator table in ${((endTime6 - endTime5) / 1000).toFixed(3)}s`);

    // Log validator execution time
    logger.info(loggerOptions, `Stored validator staking info in ${((endTime6 - startTime) / 1000).toFixed(3)}s`);
    ...
    // Log step execution time
    const endTime7 = new Date().getTime();
    logger.info(loggerOptions, `Got intentions and stakers in ${((endTime7 - endTime6) / 1000).toFixed(3)}s`);
    ...
    // Log step execution time
    const endTime8 = new Date().getTime();
    logger.info(loggerOptions, `Got intention identities in ${((endTime8 - endTime7) / 1000).toFixed(3)}s`);
    ...
    // Log step execution time
    const endTime9 = new Date().getTime();
    logger.info(loggerOptions, `Populated intention table in ${((endTime9 - endTime8) / 1000).toFixed(3)}s`);

    // Log validator execution time
    logger.info(loggerOptions, `Stored intention staking info in ${((endTime9 - endTime6) / 1000).toFixed(3)}s`);
  }
}
