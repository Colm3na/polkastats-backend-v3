...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    validatorStaking.forEach(async validator => {
      ...
      const totalBonded = validator.exposure ? BigNumber(validator.exposure.total).toString(10) : ``
      const selfBonded = validator.exposure ? BigNumber(validator.exposure.own).toString(10) : ``
      const numNominators = validator.exposure ? validator.exposure.others.length : ``
