...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    //
    const eraValidatorList = getAddressesFromExposures(exposures);
    ...
    for(let i = 0; i < validatorStaking.length; i++) {
      ...
      if (Number.isInteger(eraValidatorList.indexOf(validator.accountId))) {
        validator.currentElected = true;
      } else {
        validator.currentElected = false;
      }
      if (erasRewardPoints.individual[eraValidatorList.indexOf(validator.accountId)]) {
        validator.erasRewardPoints = erasRewardPoints.individual[eraValidatorList.indexOf(validator.accountId)];
      }
    }
    ...
    const lastSessionBlockNumber = blockNumber - sessionInfo.sessionProgress - 1; // TODO: change it for last era block number 
    ...
    const lastEraEraPoints = await api.query.staking.erasRewardPoints(lastEraIndex);
    const lastEraExposures = await api.query.staking.erasStakers.entries(currentEraIndex);
    const lastEraValidatorsList = getAddressesFromExposures(lastEraExposures);

    validatorStaking.forEach(async validator => {
      if (lastEraEraPoints.individual[lastEraValidatorsList.indexOf(validator.accountId)]) {
        const eraPoints = lastEraEraPoints.individual[lastEraValidatorsList.indexOf(validator.accountId)];
        let sqlInsert = `INSERT INTO validator_era_points (block_number, session_index, account_id, era_points, timestamp) VALUES ('${lastSessionBlockNumber}', '${lastEraIndex}', '${validator.accountId.toString()}', '${eraPoints}', extract(epoch from now()));`;
        try {
