...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    validatorStaking.forEach(async validator => {
      ...
      const producedBlocks = await api.query.imOnline.authoredBlocks(currentIndex -1, validator.accountId);
      sql = `INSERT INTO validator_produced_blocks (block_number, session_index, account_id, produced_blocks, timestamp) VALUES ('${blockNumber}', '${currentIndex}', '${validator.accountId.toString()}', 'true', extract(epoch from now()));`;
      await pool.query(sql);
