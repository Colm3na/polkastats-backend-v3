...
module.exports = {
  start: async function (api, pool, _config) {
    logger.info(`Starting staking crawler...`);
    ...
    if (res.rows.length > 0) {
      ...
      logger.info(`Last session index stored in DB is #${currentDBSessionIndex}`);
    } else {
      logger.info(`First execution, no session index found in DB!`);
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    logger.info(`Storing validators staking info for session #${currentIndex} (block #${blockNumber})`);
    ...
    if (validatorStaking) {
      ...
      try {
        ...
        logger.info(`Error inserting staking data in DB: ${JSON.stringify(error)}`);
      }
    }
    ...
    validatorStaking.forEach(async validator => {
      ...
      const totalBonded = validator.exposure ? BigNumber(validator.exposure.total).toString(10) : 0
      const selfBonded = validator.exposure ? BigNumber(validator.exposure.own).toString(10) : 0
    ...
    //
    logger.info(`Storing intentions staking info for session #${currentIndex} (block #${blockNumber}`);
    ...
    //
    logger.info(`Populating intention_staking table`);

    if (intentionStaking) {
      logger.info(`Inserting intention staking data in DB`);
      ...
      try {
        ...
        logger.info(`Error inserting intention staking data in DB: ${JSON.stringify(error)}`);
      }
    }
    ...
    //
    logger.info(`Populating intention_bonded table`);
