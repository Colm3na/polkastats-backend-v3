...
module.exports = {
  ...
  storeStakingInfo: async function (api, pool, blockNumber, sessionInfo, currentEraIndex) {
    ...
    const allNominatorAccountIdentity = allNominatorAddresses.map((accountId, index) => {
      return {
        accountId,
        identity: allNominatorIdentities[index]
      }
    });
    ...
    for(let i = 0; i < validatorStaking.length; i++) {
      ...
      for(let j = 0; j < validator.exposure.others.length; j++) {
        ...
        const identity = allNominatorAccountIdentity.find(identity => identity.accountId.toString() === nomination.who.toString());
        const displayName = identity.identity ? getDisplayName(identity.identity) : ``;
        stakers.push({
          who: nomination.who,
          displayName,
