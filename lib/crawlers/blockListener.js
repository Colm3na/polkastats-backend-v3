  ...
  start: async function (api, pool, _config) {
    ...
    await api.rpc.chain.subscribeNewHeads(async (header) => {
      ...
      if (res.rows.length > 0) {
        ...
        await storeExtrinsics(pool, blockNumber, block.extrinsics);
        ...
        await blockEvents.forEach( async (record, index) => {
          ...
          if (res.rows.length === 0) {
            ...
            try {
              await pool.query(sql);
              console.log(`[PolkaStats backend v3] - Block listener - \x1b[32m=> Added event #${blockNumber}-${index} ${event.section} => ${event.method}\x1b[0m`);
            } catch (error) {
