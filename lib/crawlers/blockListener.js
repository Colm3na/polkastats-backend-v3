// @ts-check
const { shortHash, wait } = require('../utils.js');
      ...
      if (res.rows.length > 0) {
        ...
        try {
          ...
          if (`${error}`.indexOf(`duplicate key value violates unique constraint`) !== -1) {
            ...
            console.log(`[PolkaStats backend v3] - Block listener - \x1b[31mError adding block #${blockNumber}: ${error}, sql: ${sqlInsert}\x1b[0m`);
          }
        }
          ...
          if (res.rows.length === 0) {
            ...
            try {
              ...
              if (`${error}`.indexOf(`duplicate key value violates unique constraint`) !== -1) {
                ...
                console.log(`[PolkaStats backend v3] - Block listener - \x1b[31m=> Error adding event #${blockNumber}-${index}: ${error}, sql: ${sqlInsert}\x1b[0m`);
              }
            }
          }
        });
