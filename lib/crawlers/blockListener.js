...
module.exports = {
  async blockListener(api, pool, _config) {
    console.log(
      `[PolkaStats backend v3] - \x1b[32mStarting block listener...\x1b[0m`,
    );
    ...
    await api.rpc.chain.subscribeNewHeads(async header => {
      ...
      if (res.rows.length > 0) {
        ...
        console.log(
          `[PolkaStats backend v3] - Block listener - \x1b[32mDetected chain reorganization at block #${blockNumber}, updating author, author name, hash and state root\x1b[0m`,
        );
        ...
        const sqlUpdate = `UPDATE block SET block_author = '${blockAuthor}', block_author_name = '${blockAuthorName}', block_hash = '${blockHash}', state_root = '${stateRoot}' WHERE block_number = '${blockNumber}'`;
        await pool.query(sqlUpdate);
      } else {
        // Store new block
        console.log(
          `[PolkaStats backend v3] - Block listener - \x1b[32mAdding block #${blockNumber} (${shortHash(
            blockHash.toString(),
          )})\x1b[0m`,
        );
        ...
        try {
          ...
          console.log(
            `[PolkaStats backend v3] - Block listener - \x1b[31mError: ${error}\x1b[0m`,
          );
        }
        ...
        blockEvents.forEach(async (record, index) => {
          ...
          try {
            await pool.query(sqlInsert);
            console.log(
              `[PolkaStats backend v3] - Block listener - \x1b[32m=> Adding event #${blockNumber}-${index} ${event.section} => ${event.method}\x1b[0m`,
            );
          } catch (error) {
            console.log(
              `[PolkaStats backend v3] - Block listener - \x1b[31mError adding event #${blockNumber}-${index}: ${error}, sql: ${sqlInsert}\x1b[0m`,
            );
          }
        });
      }
