...
module.exports = {
  start: async function (api, pool, _config) {
    logger.info('tarting block listener...');
    ...
    await api.rpc.chain.subscribeNewHeads(async (header) => {
      ...
      if (res.rows.length > 0) {
        ...
        logger.info(`Detected chain reorganization at block #${blockNumber}, updating author, author name, hash and state root`);
        ...
        // Store new block
        logger.info(`Adding block #${blockNumber} (${shortHash(blockHash.toString())})`);
        ...
        try {
          ...
          logger.info(`Error adding block #${blockNumber}: ${error}, sql: ${sql}`);
        }
        ...
        await blockEvents.forEach( async (record, index) => {
          ...
          if (res.rows.length === 0) {
            ...
            try {
              await pool.query(sql);
              logger.info(`Added event #${blockNumber}-${index} ${event.section} => ${event.method}`);
            } catch (error) {
              logger.info(`Error adding event #${blockNumber}-${index}: ${error}, sql: ${sql}`);
            }
          }
        });
      }
