...
module.exports = {
  start: async function (api, pool, _config) {
    ...
    const denom = denoms[_config.substrateNetwork];

    logger.info(`Starting rewards crawler...`, loggerOptions);
    ...
    if (res.rows.length > 0) {
      ...
      logger.info(`Last era index in DB is #${currentDBEraIndex}`, loggerOptions);
    } else {
      logger.info(`First execution, no era index found in DB!`, loggerOptions);
  ...
  storeEraRewardsSlashes: async function (api, pool, blockNumber, currentEraIndex, denom) {
    ...
    logger.info(`Storing rewards at block #${blockNumber} for era #${lastEraIndex}`, loggerOptions);
    ...
    for (const validator in eraValidatorSlashes) {
      ...
      try {        
        logger.info(`Adding validator slash for ${validator} in era ${lastEraIndex}: ${new BigNumber(amount).dividedBy(1e12).toNumber().toFixed(3)} ${denom}`, loggerOptions);
        await pool.query(sqlInsert);
      } catch (error) {
        logger.error(`Error adding validator slash ${JSON.stringify(error)}`, loggerOptions);
      }
    }
    ...
    for (const nominator in eraNominatorSlashes) {
      ...
      try {
        logger.info(`Adding nominator slash for ${nominator} in era ${lastEraIndex}: ${new BigNumber(amount).dividedBy(1e12).toNumber().toFixed(3)} ${denom}`, loggerOptions);
        await pool.query(sqlInsert);
      } catch (error) {
        logger.error(`Error adding nominator slash ${JSON.stringify(error)}`, loggerOptions);
      }
    }
    ...
    endEraValidatorList.forEach( async (validator, index) => {
      ...
      const poolRewardWithCommission${denom} = poolRewardWithCommission.dividedBy(1e12).toNumber().toFixed(3);
      ...
      const poolRewardWithoutCommission${denom} = poolRewardWithCommission.minus(commissionAmount).dividedBy(1e12).toNumber().toFixed(3);

      // Estimated earnings for 100 ${denom} in this era
      ...
      const estimatedPayout${denom} = estimatedPayout.dividedBy(1e12).toNumber().toFixed(3);
      ...
      const sql = `INSERT INTO rewards (block_number, era_index, stash_id, commission, era_rewards, era_points, stake_info, estimated_payout, timestamp) 
        ...
        `;
      try {
        logger.info(`Adding reward for validator ${validator}, era: ${lastEraIndex}, commission: ${commission}%, points: ${eraPoints} (${eraPointsPercent}%), exposure: ${totalExposure} ${denom}, reward: ${poolRewardWithCommission${denom}} ${denom} (without ${commission}% = ${poolRewardWithoutCommission${denom}} ${denom}), estimated era profit for 1000 ${denom}: ${estimatedPayout${denom}} ${denom}`, loggerOptions);
        await pool.query(sql);
      } catch (error) {
        logger.error(`Error adding reward for validator ${validator} and era ${lastEraIndex}`, loggerOptions);
      }
    })
    return true;
  }
