...
module.exports = {
  ...
  storeEraRewardsSlashes: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    for (const validator in eraValidatorSlashes) {
      ...
      try {        
        logger.info(`Adding validator slash for ${validator} in era ${lastEraIndex}: ${new BigNumber(amount).dividedBy(1e12).toNumber().toFixed(3)} KSM`, loggerOptions);
    ...
    for (const nominator in eraNominatorSlashes) {
      ...
      try {
        logger.info(`Adding nominator slash for ${nominator} in era ${lastEraIndex}: ${new BigNumber(amount).dividedBy(1e12).toNumber().toFixed(3)} KSM`, loggerOptions);
        await pool.query(sqlInsert);
      } catch (error) {
    ...
    endEraValidatorList.forEach( async (validator, index) => {
      ...
      const sql = `INSERT INTO rewards (block_number, era_index, stash_id, commission, era_rewards, era_points, stake_info, estimated_payout, timestamp) 
        ...
        `;
      try {
        logger.info(`Adding reward for validator ${validator}, era: ${lastEraIndex}, commission: ${commission}%, points: ${eraPoints} (${eraPointsPercent}%), exposure: ${totalExposure} KSM, reward: ${poolRewardWithCommissionKSM} KSM (without ${commission}% = ${poolRewardWithoutCommissionKSM} KSM), estimated era profit for 1000 KSM: ${estimatedPayoutKSM} KSM`, loggerOptions);
