...
async function start(api, pool, configTemplate) {
  ...
  const cmd = `${config.offlinePhragmenPath} -n ${config.substrateNetwork} --uri ${config.wsProviderUrl} staking --count 1000 --reduce ${config.phragmenOutputDir}/output.json > /dev/null`;
