...
async function start(api, pool, configTemplate) {
  ...
  logger.info('Running phragmen crawler...');
  ...
  const cmd = `${config.offlinePhragmenPath} -u ${config.wsProviderUrl} staking --output ${config.phragmenOutputDir}/output.json > /dev/null`;
