...
const DEFAULT_OFFLINE_PHRAGMEN_PATH = 'offline-election';
const DEFAULT_POLLING_TIME_MS = 10 * 60 * 1000;
 ...
 * the collected data to run "offline-election" binary and stores the output
 * on the database.
 *
...
async function start(api, pool, configTemplate) {
  ...
  await wait(DEFAULT_POLLING_TIME_MS); // run delayed
  ...
  const cmd = `${config.offlinePhragmenPath} -n ${config.substrateNetwork} --uri ${config.wsProviderUrl} staking --count 10000 -i 10 --reduce ${config.phragmenOutputDir}/output.json > /dev/null`;
