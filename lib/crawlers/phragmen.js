...
async function start(api, pool, configTemplate) {
  ...
  const cmd = `${config.offlinePhragmenPath} -n ${config.substrateNetwork} --uri ${config.wsProviderUrl} staking --count 2000 -i 10 -r ${config.phragmenOutputDir}/output.json > /dev/null`;
