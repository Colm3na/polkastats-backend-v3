...
/**
 * Fetch and store global chain counters
 *
...
async function start(api, pool) {
  ...
  await api.rpc.chain.subscribeNewHeads(async (header) => {
    ...
    const [session, blockHeight, blockHeightFinalized, totalIssuance] = await Promise.all([
    ...
    if (session.currentIndex >= currentSessionIndex) {
      ...
      if (res.rows.length > 0) {
        if (res.rows[0].session_index < currentSessionIndex) {
          ...
          insertRow(pool, blockHeight, currentSessionIndex, blockHeightFinalized, totalIssuance, activeAccounts);
        }
      } else {
        const activeAccounts = await getTotalActiveAccounts(api);
        insertRow(pool, blockHeight, currentSessionIndex, blockHeightFinalized, totalIssuance, activeAccounts);
...
async function insertRow(pool, blockHeight, currentSessionIndex, blockHeightFinalized, totalIssuance, activeAccounts) {
  const sqlInsert = 
    `INSERT INTO chain (
      block_height,
