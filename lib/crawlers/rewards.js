    ...
    console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mStoring rewards at block #${blockNumber} (session #${currentIndex})\x1b[0m`);
      ...
      rewards = await Promise.all(
        rewardEvents.map(async event => {
          ...
          let currentEraIndex = await api.query.staking.currentEra() // O.o no clue. This returns this: Option { registry: TypeRegistry {}, raw: <BN: 213> }
          console.log('current era', currentEraIndex.toString())
          let eraPoints = await api.query.staking.currentEraPointsEarned(currentEraIndex);
          let endEraValidatorList = await api.query.staking.currentElected(currentEraIndex);
