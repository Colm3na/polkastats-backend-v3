...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mTotal stake for era #${eraIndex}: ${new BigNumber(eraTotalStake).dividedBy(1e12).toNumber().toFixed(3)}\x1b[0m`);
    ...
    let eraValidatorCommission = [];
     await endEraValidatorList.forEach(async validator => {
      ...
      console.log(JSON.stringify(eraValidatorPrefs, null, 2));
      eraValidatorCommission.push({
        accountId: validator,
        commission: JSON.parse(JSON.stringify(eraValidatorPrefs.commission))
      });
    });
