...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    endEraValidatorList.forEach( async (validator, index) => {
      ...
      const exposure = new BigNumber(eraExposures.find( exposure => exposure.accountId === validator).exposure.total).dividedBy(1e12).toNumber().toFixed(3);
      ...
      console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mValidator ${validator}, era: ${eraIndex}, commission: ${eraValidatorCommission[index].commission/ 10000000 }%, points: ${eraPoints} (${eraPointsPercent}%), exposure: ${exposure} KSM, reward: ${poolRewardWithCommissionKSM} KSM\x1b[0m`)
