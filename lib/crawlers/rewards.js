...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    // Get validator commission for the last era
    const eraValidatorCommission = await endEraValidatorList.map(async validator => {
      const eraValidatorPrefs = await api.query.staking.erasValidatorPrefs(eraIndex, validator);
      return {
        accountId: validator,
        commission: eraValidatorPrefs.commission
      }
    });

    // let eraValidatorCommission = [];
    // await endEraValidatorList.forEach(async validator => {
    //   console.log(`-> validator:`, validator);
    //   const eraValidatorPrefs = await api.query.staking.erasValidatorPrefs(eraIndex, validator);
    //   console.log(`-> eraValidatorPrefs:`, JSON.stringify(eraValidatorPrefs, null, 2));
    //   eraValidatorCommission.push({
    //     accountId: validator,
    //     commission: eraValidatorPrefs.commission
    //   });
    // });
