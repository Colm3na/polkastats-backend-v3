...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentIndex) {
    ...
    console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mStoring rewards at block #${blockNumber} (session #${currentIndex})\x1b[0m`);
      ...
      rewards = await Promise.all(
        rewardEvents.map(async event => {
          ...
          // This one is giving back almost the same thing than the next one
          // let endEraValidatorList = _.map(endEraValidatorListArrays, nested => {
          //   return _.map(nested, element => {
          //     return element
          //   })
          // })
          endEraValidatorListArrays.forEach(array => {
            console.log(endEraValidatorList)
      ...
      const electedValidatorsInEra = rewards.map(({elected_validators}) => elected_validators)
      // console.log('elected', electedValidatorsInEra)
    ...
    //       VALUES ('${blockNumber}', '${currentIndex}', '${JSON.stringify(reward.stash_id)}', '${JSON.stringify(reward.eraRewards.join(''))}', '${JSON.stringify(reward.stake_info)}', extract(epoch from now()));`;
    //     try {
