...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentIndex) {
    ...
    console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mStoring rewards at block #${blockNumber} (session #${currentIndex})\x1b[0m`);
      ...
      const electedValidators = rewards.map(({elected_validators}) => elected_validators)
      let allRewards = electedValidators.map(address => {
        ...
        rewards.forEach(reward => {
          ...
          let total_stake = new BigNumber(Number(reward.points.total.toString()) / 10 ** 12); // TODO check stakeInfo. It contains exactly same info then eraPoints
