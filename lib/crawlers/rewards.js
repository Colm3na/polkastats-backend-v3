...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    endEraValidatorList.forEach( async (validator, index) => {
      ...
      const poolRewardWithCommissionKSM = new BigNumber(eraRewards).dividedBy(100).multipliedBy(eraPointsPercent).dividedBy(1e12).toNumber().toFixed(3);

      const poolRewardWithoutCommission = poolRewardWithCommission.minus(new BigNumber(eraRewards).dividedBy(100).multipliedBy(eraPointsPercent));
      ...
      const estimatedPayout = userStakeFraction.multipliedBy(poolRewardWithoutCommission).dividedBy(1e12).toNumber().toFixed(3);
