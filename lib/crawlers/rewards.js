...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentIndex) {
    ...
    console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mStoring rewards at block #${blockNumber} (session #${currentIndex})\x1b[0m`);
      ...
      // TODO: research why elected_validators are repeated so many times the exact same list
      const electedValidatorsInEra = rewards.map(({elected_validators}) => elected_validators[0]) // I don't know why it is repeated
