...
module.exports = {
  async rewards(api, pool, _config) {
    console.log(
      `[PolkaStats backend v3] - \x1b[32mStarting rewards crawler...\x1b[0m`,
    );
    ...
    await api.rpc.chain.subscribeNewHeads(async header => {
      ...
      if (res.rows.length > 0) {
        ...
        if (!crawlerIsRunning) {
          console.log(
            `[PolkaStats backend v3] - Rewards crawler - \x1b[33mFirst execution, no session index found in DB!\x1b[0m`,
          );
        }
      }
      ...
      if (currentIndex > currentDBIndex) {
        if (!crawlerIsRunning) {
          await module.exports.storeRewardsInfo(
            api,
            pool,
            blockNumber,
            currentIndex,
  ...
  async storeRewardsInfo(api, pool, blockNumber, currentIndex) {
    crawlerIsRunning = true;
    console.log(
      `[PolkaStats backend v3] - Rewards crawler - \x1b[33mStoring rewards at block #${blockNumber} (session #${currentIndex})\x1b[0m`,
    );
    ...
    console.log(
      `[PolkaStats backend v3] - Rewards crawler - \x1b[33mElected info: ${JSON.stringify(
        electedInfo,
      )})\x1b[0m`,
    );
    ...
    const response = await axios.default.get(
      `https://api-01.polkascan.io/kusama/api/v1/event?&filter[module_id]=staking&filter[event_id]=Reward`,
    );

    const rewardEvents = response.data.data;

    console.log(
      `[PolkaStats backend v3] - Rewards crawler - \x1b[33mLast rewards: ${JSON.stringify(
        rewardEvents,
      )})\x1b[0m`,
    );
    ...
    rewards = await Promise.all(
      rewardEvents.map(async event => {
        ...
        let eraPoints = await api.query.staking.currentEraPointsEarned.at(
          hash.toString(),
        );
        let endEraValidatorList = await api.query.staking.currentElected.at(
          hash.toString(),
        );
    ...
    let allRewards = await Promise.all(
      electedInfo.currentElected.map(async address => {
        const stakeInfo = electedInfo.info.find(
          val => val.stashId.toString() === address.toString(),
        );
        const commission =
          stakeInfo.validatorPrefs.commission.toNumber() / 10 ** 7;
        ...
        rewards.forEach(reward => {
          ...
          let pool_reward_with_commission =
            ((reward.reward / 100) * era_points_percent) / 10 ** 12;
          let pool_reward =
            (1 - commission / 100) * pool_reward_with_commission;
          let total_stake = new BigNumber(
            Number(stakeInfo.stakers.total) / 10 ** 12,
          );
          // Daily earning logic for frontend
          const stake_amount = new BigNumber(1000);
          const user_stake_fraction = stake_amount.div(
            total_stake.plus(stake_amount),
          );
          // Per era
          const estimated_payout = user_stake_fraction.multipliedBy(
    ...
    if (allRewards) {
      allRewards.forEach(async reward => {
        let sqlInsert = `INSERT INTO rewards (block_number, session_index, stash_id, era_rewards, commission, stake_info, timestamp) 
          VALUES ('${blockNumber}', '${currentIndex}', '${JSON.stringify(
          reward.stash_id,
        )}', '${JSON.stringify(reward.eraRewards.join(''))}', '${
          reward.commission
        }', '${JSON.stringify(
          reward.stake_info,
        ...
        try {
          ...
          console.log(
            `[PolkaStats backend v3] - Rewards crawler - \x1b[33mResponse from Database is ${JSON.stringify(
              res,
            )}]`,
          );
        } catch (error) {
          console.log(
            `[PolkaStats backend v3] - Rewards crawler - \x1b[31mSQL: ${sqlInsert}\x1b[0m`,
          );
          console.log(
            `[PolkaStats backend v3] - Rewards crawler - \x1b[31mERROR: ${JSON.stringify(
              error,
            )}\x1b[0m`,
          );
        }
