...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    // Get validator preferences for the last era
    const eraValidatorPrefs = endEraValidatorList.map(async validator => {
      const eraValidatorPrefs = await api.query.staking.erasValidatorPrefs(eraIndex, validator);
      return {
        accountId: validator,
        validatorPrefs: JSON.parse(JSON.stringify(eraValidatorPrefs))
      }
    });
    console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mValidator preferences for era #${eraIndex}: ${JSON.stringify(eraValidatorPrefs, null, 2)}\x1b[0m`);
