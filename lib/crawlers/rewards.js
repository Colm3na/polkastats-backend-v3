  ...
  rewards: async function (api, pool) {
    // Subscribe to new blocks
      await api.rpc.chain.subscribeNewHeads( async (header) => {
        ...
        const currentSessionIndex = session.currentIndex.toNumber();
        // console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[32mCurrent session index is #${currentSessionIndex}\x1b[0m`);
        ...
        // get all validator addresses who got elected (for rewards) in this era
        ...
        if (currentSessionIndex > currentDBIndex) {
          if (!crawlerIsRunning) {
            await module.exports.storeRewardsInfo(api, pool, blockNumber, endEraValidatorList, currentSessionIndex, currentEraIndex);
          }
        }
    });
  },
  storeRewardsInfo: async function (api, pool, blockNumber, endEraValidatorList, currentSessionIndex, currentEraIndex) {
    crawlerIsRunning = true;
    console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33mStoring rewards at block #${blockNumber} (session #${currentSessionIndex})\x1b[0m`);
      ...
      let allRewards = await Promise.all(
        endEraValidatorList.map(async address => {
        const eraIndex = parseInt(currentEraIndex)
        const erasValidatorPrefs = await api.query.staking.erasValidatorPrefs(eraIndex, address)
