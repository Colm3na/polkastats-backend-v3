...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    endEraValidatorList.forEach( async (validator, index) => {
      ...
      const estimatedPayout = userStakeFraction.multipliedBy(poolRewardWithoutCommission).dividedBy(1000000000000).toNumber().toFixed(6);
      ...
      // const perValidatorReward = new BigNumber(eraRewards).dividedBy(endEraValidatorList.length);
      // const validatorPayment = new BigNumber(commission).multipliedBy(perValidatorReward);
      // const rewardSplit = perValidatorReward.minus(validatorPayment);

      // const rewardPayout = stakeAmount.multipliedBy(rewardSplit).dividedBy(stakeAmount.plus(new BigNumber(exposure.total)));
      // const rewardPayoutKSM = rewardPayout.dividedBy(1000000000000).toNumber().toFixed(6);
