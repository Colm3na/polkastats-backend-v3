...
module.exports = {
  ...
  storeRewardsInfo: async function (api, pool, blockNumber, currentEraIndex) {
    ...
    endEraValidatorList.forEach( async (validator, index) => {
      ...
      // Daily earning logic for 100 KSM
      const stakeAmount = new BigNumber(100 * 1e12);
      const userStakeFraction = stakeAmount.dividedBy(eraTotalStake.plus(stakeAmount));
      const estimatedPayout = userStakeFraction.multipliedBy(poolRewardWithCommission).dividedBy(1e12).toNumber().toFixed(3);
      ...
      try {
        console.log(`[PolkaStats backend v3] - Rewards crawler - \x1b[33m=> Adding reward for validator ${validator}, era: ${eraIndex}, commission: ${eraValidatorCommission[index].commission / 10000000 }%, points: ${eraPoints} (${eraPointsPercent}%), exposure: ${totalExposure} KSM, reward: ${poolRewardWithCommissionKSM} KSM, estimated payout: ${estimatedPayout}\x1b[0m`);
