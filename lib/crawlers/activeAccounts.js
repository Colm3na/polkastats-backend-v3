...
const makeQuery = (state, block, timestamp) => {
  ...
  const JSONbalances = JSON.stringify(balances.balances); 
  const query = `                                                                                                                                  \
      INSERT INTO   account (account_id, identity, balances, available_balance, free_balance, timestamp, block_height, is_staking)                 \
      VALUES        ('${id}', '${identity}', '${JSONbalances}', '${availableBalance}', '${freeBalance}', '${timestamp}', '${block}', ${isStaking}) \
...
const start = async (api, pool, config) => {
  ...
  (async function run() {
    await exec(api, pool).catch(err =>
      logger.error({ msg: 'Error running crawler', err }),
    );

    setTimeout(() => run(api, pool, config), pollingTime);
  })();
};
