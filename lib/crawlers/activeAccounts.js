module.exports = {
  activeAccounts: async function (api, pool, config) {
    ...
    for (var key in accounts ) {
      ...
      console.log(`[PolkaStats backend v3] - Active Accounts - \x1b[33mProcessing account ${accountId}`);
    }
    ...
    for (var key in accountsInfo ) {
      if (accountsInfo.hasOwnProperty(key)) {
        ...
        if (res.rows.length > 0) {
          ...
          try {
            console.log(`[PolkaStats backend v3] - Active Accounts - \x1b[33mUpdating account ${accountsInfo[key].accountIndex} [${key}]`);
          ...
          try {
            console.log(`[PolkaStats backend v3] - Active Accounts - \x1b[33mAdding account ${accountsInfo[key].accountIndex} [${key}]`);
            await pool.query(sql);
          } catch (error) {
