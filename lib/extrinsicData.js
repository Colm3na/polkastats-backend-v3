...
function get ({
  ...
  if (
    ...
    if (ownerData) {
      const owner = ownerData['id'] || ownerData['substrate'] || ownerData['ethereum'] || null;
      prepareData.to_owner = normalizeSubstrateAddress(owner);
    }
  }
  ...
  function getSigned() {
    ...
    result.signer = isSigned ? normalizeSubstrateAddress(extrinsic.signer.toString()) : "";
    return result;
  }
