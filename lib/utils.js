...
module.exports = {
  ...
  storeEraRewardsSlashes: async function (api, pool, eraIndex, denom) {
    ...
    for (const validator in eraValidatorSlashes) {
      ...
      const sqlInsert = `INSERT INTO validator_slash_era (era_index, account_id, amount, timestamp) 
    ...
    for (const nominator in eraNominatorSlashes) {
      ...
      const sqlInsert = `INSERT INTO nominator_slash_era (era_index, account_id, amount, timestamp) 
    ...
    for(let i = 0; i < endEraValidatorList.length; i++) {
      let validator = endEraValidatorList[i];
    ...
    endEraValidatorList.forEach( async (validator, index) => {
      ...
      const sql = `INSERT INTO reward (era_index, stash_id, identity, display_name, commission, era_rewards, era_points, stake_info, estimated_payout, timestamp) 
        VALUES ('${eraIndex}', '${validator}', '${JSON.stringify(validator.identity)}', '${validator.displayName}', '${eraValidatorCommission[index].commission}', '${poolRewardWithCommission.toString(10)}', '${eraPoints}', '${JSON.stringify(exposure)}', '${estimatedPayout.toFixed(0)}', '${Date.now()}')
        ON CONFLICT ON CONSTRAINT reward_pkey 
        DO NOTHING
        `;
      try {
        logger.info(loggerOptions, `Adding reward for validator ${validator} and era: ${eraIndex}, commission: ${commission}%, points: ${eraPoints} (${eraPointsPercent}%), exposure: ${totalExposure} ${denom}, reward: ${poolRewardWithCommissionKSM} ${denom} (without ${commission}% = ${poolRewardWithoutCommissionKSM} ${denom}), estimated era profit for 1000 ${denom}: ${estimatedPayoutKSM} ${denom}`);
        await pool.query(sql);
      } catch (error) {
        logger.error(loggerOptions, `Error adding reward for validator ${validator} and era ${eraIndex}: ${JSON.stringify(error)}`);
      }
    })
    return true;
  }
