...
module.exports = {
  ...
  storeEraStakingInfo: async function (api, pool, eraIndex, denom, loggerOptions) {
    ...
    const eraValidatorSlashes = JSON.parse(JSON.stringify(eraSlahes.validators));
    const eraNominatorSlashes = JSON.parse(JSON.stringify(eraSlahes.nominators));

    endEraValidatorList.forEach( async (validator, index) => {
      if (eraValidatorSlashes.includes(validator)) {
