...
module.exports = {
  ...
  storeExtrinsics: async function (pool, blockNumber, extrinsics, blockEvents) {
    ...
    extrinsics.forEach(async (extrinsic, index) => {
      ...
      const success = module.getExtrinsicSuccess(index, blockEvents)

      logger.info(`Parsed extrinsic ${blockNumber}-${index} isSigned: ${isSigned} signer: ${signer} section: ${section} method: ${method} args: ${args} hash: ${hash} success: ${success}`);
