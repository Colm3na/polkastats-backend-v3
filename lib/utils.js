...
module.exports = {
  ...
  storeEraStakingInfo: async function (api, pool, eraIndex, denom) {
    ...
    for (const validator in eraValidatorSlashes) {
      ...
      const sqlInsert = `INSERT INTO validator_era_slash (era_index, account_id, amount, timestamp) 
    ...
    for (const nominator in eraNominatorSlashes) {
      ...
      const sqlInsert = `INSERT INTO nominator_era_slash (era_index, account_id, amount, timestamp) 
