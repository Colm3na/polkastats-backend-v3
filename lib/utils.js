  ...
  updateTotals: async function (pool, loggerOptions) {
    let sql = `
      SELECT
        ...
        (SELECT count(*) FROM extrinsic) AS extrinsics,
        (SELECT count(*) FROM event) AS events;
    `;
    const res = await pool.query(sql);
    if (res.rows.length > 0) {
      logger.info(loggerOptions, `Rows: (${JSON.stringify(res.rows)})`);
      const blocks = res.rows[0]["blocks"];
      const extrinsics = res.rows[0]["extrinsics"];
      const events = res.rows[0]["events"];
      sql = `
        UPDATE total SET count = '${blocks}' WHERE name = 'blocks';
        UPDATE total SET count = '${extrinsics}' WHERE name = 'extrinsics';
        UPDATE total SET count = '${events}' WHERE name = 'events';
      `;
      try {
        logger.info(loggerOptions, `Updating total harvested blocks (${blocks}), extrinsics (${extrinsics}) and events (${events})`);
        await pool.query(sql);
      } catch (error) {
