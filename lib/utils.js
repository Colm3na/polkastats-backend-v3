...
module.exports = {
  ...
  updateTotals: async function (pool, loggerOptions) {
    ...
    if (res.rows.length > 0) {
      ...
      sql = `
        UPDATE total SET amount = '${totalBlocks}' WHERE NAME = 'blocks';
        UPDATE total SET amount = '${totalExtrinsics}' WHERE NAME = 'extrinsics';
        UPDATE total SET amount = '${totalEvents}' WHERE NAME = 'events';
      `;
      try {
