...
class BackendV3 {
  ...
  async runCrawlers() {
    ...
    this.subscribeToNewBlocks(api, pool);
  ...
  async subscribeToNewBlocks(api, pool) {
    ...
    await api.rpc.chain.subscribeNewHeads(async (header) => {
      ...
      if (res.rows.length > 0) {
        ...
        try {
          ...
          console.log(`[PolkaStats backend v3] - Block listener - \x1b[33mError: ${error}\x1b[0m`);
        }
  ...
  async blockHarvester(api, pool) {
    ...
    for (let i = 0; i < res.rows.length; i++) {
      ...
      if (!(res.rows[i].gap_start == 0 && res.rows[i].gap_end == 0)) {
        ...
        await this.harvestBlocks(api, pool, parseInt(res.rows[i].gap_start), parseInt(res.rows[i].gap_end));
      }
    }
    ...
    setTimeout(this.blockHarvester, this.config.BLOCK_HARVESTER_POLLING_TIME);
  ...
  async harvestBlocks(api, pool, startBlock, endBlock) {
    ...
    while (startBlock <= endBlock) {
      ...
      blockEvents.forEach( async (record, index) => {
        ...
        try {
          await pool.query(sqlInsert);
          console.log(`[PolkaStats backend v3] - Block harvester - \x1b[33mAdding event #${endBlock}-${index} ${event.section} => ${event.method}\x1b[0m`);
        } catch (error) {
          console.log(`[PolkaStats backend v3] - Block harvester - \x1b[31mError adding event #${endBlock}-${index}: ${error.error}\x1b[0m`);
        }
      });
