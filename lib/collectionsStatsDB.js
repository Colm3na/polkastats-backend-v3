...
async function increaseStatsValue(sequelize, collection_id, fieldName) {
  const collectionStats = await getCollectionStatsById(sequelize, collection_id);
  ...
  if (collectionStats) {
    await updateStatsRecord(
      ...
      collection_id,
      fieldName,
      Number(collectionStats[fieldName]) + 1,
    );
  } else {
...
async function increaseActionsCount(sequelize, event) {
  if (collectionMethods.includes(event.method)) {
...
async function decreaseTokensCount(sequelize, collection_id) {
  ...
  await updateStatsRecord(
    ...
    collection_id,
    'tokens_count',
    Number(collectionStats.tokens_count) - 1,
  );
}
...
async function decreaseHoldersCount(sequelize, collection_id, token_id) {
  const collectionStats = await getCollectionStatsById(sequelize, collection_id);
  const tokens = await sequelize.query(
    `select token_id from tokens t
    where collection_id = :collection_id and "owner" in
    (select "owner" from tokens where token_id = :token_id and collection_id = :collection_id )`, {
      type: QueryTypes.SELECT,
      ...commonQueryOptions,
      replacements: {
        collection_id,
  ...
  if (!hasAnotherToken && collectionStats) {
    await updateStatsRecord(
      sequelize,
      collection_id,
      'holders_count',
      Number(collectionStats.holders_count) - 1,
    );
  }
}
